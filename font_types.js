...
function PCFTable (parent, type, format, data) {
  ...
  switch (type) {
    case 'properties':
      if (this.format !== data.readUInt32LE(0))
...
function PCF (data) {
  ...
  var tables = this.data.readUInt32LE(4) * 16;
  this.tables = [];
  this.characters = [];
  for (var i = 0 + 8; i < tables + 8; i += 16) {
    var type_id = this.data.readUInt32LE(i)
      , type    = _pcf_tables.filter(function (type, index) { return type_id === 1 << index })[0]
      , format = this.data.readUInt32LE(i +  4)
      , size   = this.data.readUInt32LE(i +  8)
      , offset = this.data.readUInt32LE(i + 12)
      , end    = offset + size;
    this.tables.push(new PCFTable(this, type, format, this.data.slice(offset, Math.min(end, this.data.length))));
  }
